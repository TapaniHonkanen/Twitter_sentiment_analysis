...
# and later appended by Jari Jussila, HAMK University of Applied Sciences
...
# Create local webserver and auto handles authentication for Google Drive later on
gauth = GoogleAuth()
...
filename = "A direct link to your TAGS twitter data collection sheet. Point it specifically to Archive, and set to .csv"
...
output = "NAMEHERE_tags_output.csv"
output2 = "NAMEHERE_tags_mentions.csv"
output3 = "NAMEHERE_tags_hashtags.csv"
...
# UPLOAD CREATED CSV FILES TO GOOGLE DRIVE
drive = GoogleDrive(gauth)

# define destination folder id on google drive
folder_id = 'Your Google Drive folder id here'

# get a list of existing files in the folder and delete all existing files (could be done also by updating file contents through file id's)
myTwitterFiles = drive.ListFile({'q': "'Your Google Drive folder id here' in parents"}).GetList()
for file1 in myTwitterFiles:
  file1.Delete()

# create instances of new files to be uploaded to the destination folder
fil1 = drive.CreateFile({"parents": [{"kind": "drive#fileLink", "id": folder_id}]})
fil2 = drive.CreateFile({"parents": [{"kind": "drive#fileLink", "id": folder_id}]})
fil3 = drive.CreateFile({"parents": [{"kind": "drive#fileLink", "id": folder_id}]})

# define which files are to be uploaded
fil1.SetContentFile('NAMEHERE_tags_output.csv')
fil2.SetContentFile('NAMEHERE_tags_mentions.csv')
fil3.SetContentFile('NAMEHERE_tags_hashtags.csv')
